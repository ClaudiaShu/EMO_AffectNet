...
class RandomLandmarkMask(object):
    ...
    def __init__(self, p=0.8):
        ...
        cnn_detector_path = "/data/users/ys221/data/pretrain/mix_models/mmod_human_face_detector.dat"
        face_predictor_path = "/data/users/ys221/data/pretrain/mix_models/shape_predictor_68_face_landmarks.dat"
            ...
            if isinstance(clip[0], np.ndarray):
                ...
                for i in range(len(clip)):
                    ...
                    if landmark is None:
                        ...
                        radius = int(np.round((maxy - miny) / 2)+offset)
                        mask = cv2.circle(mask, centre, radius, (255, 255, 255), -1)
                        clip[i] = np.where(mask == (0, 0, 0), image, blurred_img)
                return np.array(clip)
            elif isinstance(clip[0], PIL.Image.Image):
                # image open with PIL
                # TODO: change black mask to Gaussian mask in PIL
                ...
                for image in clip:
                    ...
                    draw.ellipse(twoPointList, fill=(255, 255, 255, 0))

            # elif isinstance(clip[0], torch.Tensor):
            #     clip = clip.permute(1, 2, 3, 0).detach().numpy() # CTHW to THWC
            #     for i in range(len(clip)):
            #         image = clip[i]
            #         landmark = F.detect_landmark(image, self.detector, self.cnn_detector, self.face_predictor)
            #         if landmark is None:
...
class RandomGaussianBlur(object):
    ...
    def __call__(self, clip):
        if random.random() < self.p:
            if isinstance(clip[0], np.ndarray):
                return [cv_GB(img,3,0) for img in clip]
            elif isinstance(clip[0], PIL.Image.Image):
                return [
